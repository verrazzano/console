...
import { BindingComponent, ComponentType, Status } from 'vz-console/service/loader'
...
async function setup(components: BindingComponent[]) {
  fixture.set(`<div id ="component-holder"><vz-console-binding-components id="binding-components" components="[[components]]"/></div>`);
  componentsElement = document.querySelector('#binding-components') as HTMLElement;
...
describe('generic binding component tests', () => {
  ...
  it('renders the generic component correctly.', async () => {
    ...
    expect(componentsElement.querySelector(`#${component.id}_ns`).textContent).to.equal(component.placement.namespace)
    expect(componentsElement.querySelector(`#${component.id}_cluster`).textContent).to.equal(component.placement.cluster)
    expect(componentsElement.querySelector(`#${component.id}_status`).textContent.trim()).to.equal(component.status)
