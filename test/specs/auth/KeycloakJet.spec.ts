...
import { KeycloakJet, Keycloak, KeycloakUrls, KeycloakJwt } from 'vz-console/auth/loader';
...
describe('initAuth tests', () => {
    ...
    const stubIsReturnFromKeycloak = sinon.stub(Keycloak, 'isReturnFromKeycloakLogin');
    const stubLocationReplace = sinon.stub(Keycloak, 'replaceWindowLocation').callsFake((loc) => {
    ...
    sinon.stub(keycloakUrls, 'getAuthUrlPrefix').returns("https://fakeKeycloakUrl");
    ...
    it('calls keycloak when no access token', async () => {
        ...
        await delay(2000); // Wait 2 seconds - there doesn't seem to be a cleaner way to do the wait for the redirect call, which returns a Promise that never resolves
        console.log(`location replace called ${stubLocationReplace.callCount} times`);
        expect(stubLocationReplace.callCount).to.gte(1);
        const actualRedirectLocation = stubLocationReplace.args[stubLocationReplace.callCount-1][0] as string;
        expect(actualRedirectLocation).to.satisfy(
            loc => loc.startsWith("https://fakeKeycloakUrl") && loc.includes("client_id=fakeClientId"),
    ...
    it('fetches and stores token when returning from Keycloak', async () => {
        ...
        sinon.stub(KeycloakJwt, 'calcExpiryTsMillis').returns(Date.now() + 60*60*1000);
        
        const fakeTokenResponse = {status: 200, json: async () => { return Promise.resolve({access_token: 'myaccesstoken', refresh_token: 'myrefreshtoken'}); }} as Response;
...
describe('authenticatedFetch', () => {
    it('adds bearer token', async () => {
        ...
        const stubAccessTokenExists = sinon.stub(KeycloakJwt, 'isAccessTokenExpired').returns(false);
        ...
        const req = stubWindowFetch.args[stubWindowFetch.callCount-1][0] as Request;
        expect(req).ok; // req is not null/undefined
        expect(req.headers.get('Authorization')).to.equal("Bearer fakeAccessTokenHere");
    });
});