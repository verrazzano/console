...
export class HeaderBar {
    ...
    public async selectLogo(): Promise<boolean> {
        const logo = await
            Wait.waitForPresent(HeaderBar.LOGO);
    ...
    public async selectUserMenu(): Promise<boolean> {
        const userMenuButton = await
            Wait.waitForPresent(HeaderBar.USER_MENU_BUTTON);
    ...
    public async selectUserMenuContent(): Promise<boolean> {
        const userMenuContent = await Wait.waitForPresent(HeaderBar.USER_MENU_CONTENT);
