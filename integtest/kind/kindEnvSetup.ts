...
const deployVerrazzanoOperator = async (kubeClient: KubeClient) => {
    ...
    if (!deployment) {
        throw new Error(`No Deployment found in verrazzano-operator YAML at ${actualOperatorManifest}, which is based on template ${verrazzanoOperatorHelmTemplate}`);
    }
    const containersYaml = deployment['spec']['template']['spec']['containers'] as any[];
    if (!containersYaml || containersYaml.length != 1) {
        throw new Error(`Expected exactly one container on the verrazzano-operator deployment ${actualOperatorManifest}!`);
    }
    ...
    if (containersYaml[0]['args']) {
        ...
        throw new Error(`Did not find args section on the container in the verrazzano-operator deployment in ${actualOperatorManifest}`)
    }
    ...
    const editedOperatorManifest = path.join(tempDir, 'verrazzano-operator-modified.yaml');
    ...
    console.log(`Applying verrazzano-operator manifest ${editedOperatorManifest}`);
...
const setupIntegTestEnv = async () => {
    try {
        ...
        if (kubeConfigFile) {
            console.log(`Using existing KinD cluster as specified in KIND_KUBECONFIG env var: ${kubeConfigFile}\n`);
        } else {
            ...
            kubeConfigFile = await KindUtil.createKindCluster(clusterName, kindConfigFile);
        }
...
const getVerrazzanoOperatorURL = async (kubeClient: KubeClient): Promise<string> => {
    try {
        const nodePort = await kubeClient.getServiceNodePort('verrazzano-operator', vzNamespace);
        const nodeAddr = await kubeClient.getNodeInternalIPAddr(`${clusterName}-worker`);
