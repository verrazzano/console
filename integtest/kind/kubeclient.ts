...
export class KubeClient {
  ...
  constructor(kubeConfigFile: string) {
    if (!kubeConfigFile) {
      throw new Error(
        "Could not initialize Kubernetes client - no kube config file specified!"
      );
    }
  ...
  async createDockerRegistrySecret(
    ...
    try {
      const result = await this.k8sApi.createNamespacedSecret(
        secretNamespace,
        theSecret
      );
      console.log(
        `Created secret ${secretName}, response status is ${result.response.statusCode}`
      );
    } catch (err) {
      console.log(
        `Failed to create Docker Registry secret ${secretName}: ${err}`
      );
      throw err;
    }
  }
  ...
  async getNodeInternalIPAddr(nodeName: string): Promise<string> {
    ...
    const nodeAddr = nodeResp.body.status.addresses.find(
