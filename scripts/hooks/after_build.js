...
module.exports = function (configObj) {
  return new Promise((resolve, reject) => {
    ...
    const ojLibVersion = filesystem
      .readdirSync(ojLibDir)
      .find(
        (name) =>
          name.match(versionedDirRegex) &&
          filesystem.lstatSync(`${ojLibDir}/${name}`).isDirectory()
      );
    console.log(
      `Oracle JET library version is ${ojLibVersion}, creating symlink ${symlinkPath} to this version for use by test/test-main.js`
    );
